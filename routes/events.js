 ...
 * event_other    Viewing other project's page
 * event_own      Viewing own project page
 */
router.use('/:id?\*', function(req, res, next) {
  ...
  req.page.event.getProjects().then(function(members) {

    if (req.page.props.auth) {
      req.page.props.event_other = true;
      req.page.props.event_own = false;
      for (var i in members) {
        if (members[i].id === req.user.id) {
          req.page.props.event_own = true;
          req.page.props.event_other = false;
          break;
        }
      }
    }
...
router.use('/:id/', require('./event/home'));
